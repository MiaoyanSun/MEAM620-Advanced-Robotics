...
angular_velocity = imu0[:, 1:4]   # column 1 2 3

# linear acceleration in meters per second^2
linear_acceleration = imu0[:, 4:]  # column 4 till the end

# %% Process the imu data

n = imu0.shape[0]   # total rows

euler = np.zeros((n, 3))

R = Rotation.identity()
for i in range(1, n):
    dt = (t[i] - t[i - 1]) * 1e-9  # change nanoseconds to seconds
